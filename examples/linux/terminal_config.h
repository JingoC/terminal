...
#define TERM_KEY_ENTER					(_KEY_INIT(0x0A))		// Enter command symbol
...
#define COM_Printf(...)		{sprintf(dbgbuffer,__VA_ARGS__); printf("%s", dbgbuffer); fflush(stdout);}
#define CLI_Printf			COM_Printf
#if (ECHO_EN == 1)
#define CLI_PutChar(c)			{ char ch = c; write(STDOUT_FILENO, &ch, 1); }
#else	// ECHO_EN != 1
#define CLI_PutChar
#endif	// ECHO_EN == 1
...
// ************************ Time calculate Settings *************************
	...
	// your implementation
extern long long TimeUs(void);
extern long long TimeMs(void);
extern void TimeReset(void);

#define CLI_GetUs()					(TimeUs())	// System time in us
#define CLI_GetFastUs()				(TimeUs())			// System time in us (not exact)
#define CLI_GetFastMs()				(TimeMs())			// System time in ms (not exact)
